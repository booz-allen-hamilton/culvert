...
/**
 * Creates an index based on url values to exploit url locality. Each
 * value is stored in the row ID based on
 * <tt>&lt;term>&lt;6_nulls>&lt;data_row_id>&lt;data_row_id_length_in_4_bytes></tt>
 * .
 * 
 */
public class UrlBasedIndex extends Index {

  private static final Log LOG = LogFactory.getLog(UrlBasedIndex.class);

  // Configuration settings
  private static final String TOKEN_REGEX_KEY = "culvert.index.term.token";

  /**
   * Regex used to find gtld's.
   * <p>
   * Currently set to: .{1}[(com)(gov)(edu)(biz)(info)(mil)(org)(net)]{1}
   */
  public static final String DEFAULT_TOKEN_REGEX = ".{1}[(com)(gov)(edu)(biz)(info)(mil)(org)(net)]{1}";
   ...
   * @param indexTable
   */
  public UrlBasedIndex(String name, DatabaseAdapter database,
  ...
  public UrlBasedIndex(String name, DatabaseAdapter database,
      ...
      byte[] columnFamily, byte[] columnQualifier, String regex) {
    super(name, columnFamily, columnQualifier, database, databaseConf,
        primaryTable, indexTable);
  }
  
  /**
   * Sets the regex used to split the value into tokens. 
  ...
  public SeekingCurrentIterator handleGet(byte[] indexRangeStart,
    ...
    return new UrlBasedIndexIterator(indexTable.get(get));
  }
  ...
  private List<CKeyValue> createTermList(CKeyValue keyValue)
    ...
    byte[] indexRowId;
	// Modify the term bytes. Find a pattern match and prepend to the term.
	try {
	  Pattern pattern = Pattern.compile(tokenRegex);
	  Match matcher = pattern.matcher(value);
	  
	  if(matcher.find()){
		value = matcher.group().substring(1) + value;
	  }
   ...
   * 
   */
  public static class UrlBasedIndexIterator extends DecoratingCurrentIterator {

    public UrlBasedIndexIterator(SeekingCurrentIterator iterator) {
      super(iterator);
    }
    ...
    public Result next() {
      ...
      byte[] dataRowId = UrlBasedIndex.parseIndexRowGetId(indexRowId);
