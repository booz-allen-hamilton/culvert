...
/**
 * Creates an index based on float/double values to allow for sorting. Just stores the binary value, 
 * but flips the sign bit of positive numbers and inverts negative numbers so
 * we sort properly. Each index
 * value is stored in the row ID based on
 * <tt>&lt;term>&lt;6_nulls>&lt;data_row_id>&lt;data_row_id_length_in_4_bytes></tt>
 * .
 * 
 */
public class FloatBasedIndex extends Index {

  private static final Log LOG = LogFactory.getLog(FloatBasedIndex.class);
   ...
   * @param indexTable
   */
  public FloatBasedIndex(String name, DatabaseAdapter database,
  ...
  public SeekingCurrentIterator handleGet(byte[] indexRangeStart,
    ...
    return new FloatBasedIndexIterator(indexTable.get(get));
  }
  ...
  private List<CKeyValue> createTermList(CKeyValue keyValue)
    ...
    byte[] indexRowId;
	// Modify the term bytes. For negative numbers, invert all bits.
	// For positive numbers, invert the sign bit.
	byte[] term = Bytes.toBytes(value);
	if(term.length == Float.SIZE / 8){
	  float iTerm = Bytes.toFloat(term);
	  if(iTerm < 0){
  		for(int i = 0; i < term.length; term++){
		  	term[i] ^= 0xFF;
		}
	  }else{
		term[0] ^= 0x80;
	  }
      indexRowId = createIndexRowId(term, recordTableRowId);
      indexKeyValueList.add(new CKeyValue(indexRowId));
	}else{
	  double iTerm = Bytes.toDouble(term);
	  if(iTerm < 0){
  		for(int i = 0; i < term.length; term++){
		  	term[i] ^= 0xFF;
		}
   ...
   * 
   */
  public static class FloatBasedIndexIterator extends DecoratingCurrentIterator {

    public FloatBasedIndexIterator(SeekingCurrentIterator iterator) {
      super(iterator);
    }
    ...
    public Result next() {
      ...
      byte[] dataRowId = FloatBasedIndex.parseIndexRowGetId(indexRowId);
